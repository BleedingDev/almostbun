 ...
 * Version: 16 - add app-shell caching for repo runner while preserving virtual routing
 */

const DEBUG = false;
const SHELL_CACHE_NAME = 'almostbun-shell-v1';
const RUNNER_FALLBACK_PATH = '/examples/repo-runner.html';
const RUNNER_PATH_ALIASES = new Set([
  '/examples/repo-runner',
  '/examples/repo-runner/',
  '/examples/repo-runner.html',
]);
...
function isRunnerNavigationPath(pathname) {
  return RUNNER_PATH_ALIASES.has(pathname);
}

function isRunnerShellAssetRequest(request, url) {
  if (request.method !== 'GET' || !isSameOrigin(url)) {
    return false;
  }

  if (url.pathname === '/repo-runner.webmanifest') {
    return true;
  }
  ...
  const isAssetDestination = destination === 'script' ||
    destination === 'style' ||
    destination === 'font' ||
    destination === 'image' ||
    destination === 'manifest';

  if (!isAssetDestination) {
    return false;
  }

  if (request.referrer && request.referrer.includes('/examples/repo-runner')) {
    return true;
  }

  // In production build assets are hashed and may not include runner in filename.
  // We still cache /assets/* resources as long as they belong to this origin.
...
async function precacheRunnerShell() {
  const cache = await caches.open(SHELL_CACHE_NAME);
  await Promise.all(
    PRE_CACHE_PATHS.map(async (path) => {
      try {
        ...
        // Ignore pre-cache failures (offline first visit).
      }
    })
  );
}

async function handleRunnerNavigationRequest(request) {
  const cache = await caches.open(SHELL_CACHE_NAME);
  try {
    ...
    const fallback = await cache.match(RUNNER_FALLBACK_PATH);
    if (fallback) {
      return fallback;
    }
    return new Response('Offline: repo runner shell is not cached yet.', {
      status: 503,
...
async function handleRunnerShellAssetRequest(request) {
  const cache = await caches.open(SHELL_CACHE_NAME);
  const cachedResponse = await cache.match(request);
...
self.addEventListener('fetch', (event) => {
  ...
  if (!match) {
    // Repo Runner app-shell caching
    if (isSameOrigin(url) && event.request.method === 'GET') {
      if (event.request.mode === 'navigate' && isRunnerNavigationPath(url.pathname)) {
        event.respondWith(handleRunnerNavigationRequest(event.request));
        return;
      }

      if (isRunnerShellAssetRequest(event.request, url)) {
        event.respondWith(handleRunnerShellAssetRequest(event.request));
        return;
      }
    }
...
self.addEventListener('activate', (event) => {
  ...
  event.waitUntil((async () => {
    ...
    await Promise.all(
      cacheNames
        .filter(name => name.startsWith('almostbun-shell-') && name !== SHELL_CACHE_NAME)
